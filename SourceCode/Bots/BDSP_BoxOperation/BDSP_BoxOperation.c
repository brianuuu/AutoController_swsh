		...
		case PROCESS:
			...
			if (commandIndex == -1)
			{
				...
				switch (m_sequence)
				{
					...
					case 1: // SS_Start
					{
						if (m_operationType == 0) // release
						{
							commandIndex = 6;
							m_endIndex = 13;
						}
						else if (m_operationType == 1) // bag item
						{
							commandIndex = 14;
							m_endIndex = 19;
						}
					...
					case 2: // SS_NextPokemon
					{
						// Back to per pokemon operation
						m_sequence = 0;
						
						if (m_posX == 6 && m_posY == 5)
						{
							if (m_operationType == 2) // duplicate item
							{
								// Exit 2nd menu
								commandIndex = 25;
								m_endIndex = 34;
